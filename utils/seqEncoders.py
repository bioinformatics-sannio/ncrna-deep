...
def seq2Snake(seq: Seq, sdim=[[16,16],IUPAC.IUPACAmbiguousDNA.letters],padding='constant'):
    ...
    if (padding=='random'):
        seqr=np.random.randint(len(sdim[1]), size=sdim[0])
    if (padding=='new'):
        char_to_int = dict((c, i+1) for i, c in enumerate(sdim[1]))
        seqr=np.zeros(sdim[0])

    #ss = np.empty(sdim[0],dtype=np.str)
    k = 0
    j = 0
    for i,c in enumerate(str(seq)):
        if k==sdim[0][0]:
            k=0
            j +=1
        seqr[j,k] = char_to_int[c]
...
def seq2Hilbert(seq: Seq, sdim=[[16,16],IUPAC.IUPACAmbiguousDNA.letters],padding='constant'):
    ...
    hilbert_curve = HilbertCurve(int(np.ceil(np.sqrt(sdim[0][0]))),2)
    #ss = np.empty(sdim[0],dtype=np.str)
    for i,c in enumerate(str(seq)):
        coords = hilbert_curve.coordinates_from_distance(i)
        seqr[coords[0],coords[1]] = char_to_int[c]
...
def seq2Morton(seq: Seq, sdim=[[16,16],IUPAC.IUPACAmbiguousDNA.letters],padding='constant'):
    ...
    #ss = np.empty(sdim[0],dtype=np.str)
    for i,c in enumerate(str(seq)):
        coords = pymorton.deinterleave2(i)
        seqr[coords[0],coords[1]] = char_to_int[c]
        #ss[coords[0],coords[1]] = c
    return seqr
...
def encode_seqs(seqlist,enc=seq2Snake,encparam=[[13,13]],padding='constant'):
...
if __name__ == "__main__":
    ...
    w=seq2Simple(q,sdim=[[20],2,j])
    print(w)
    print(len(w))
    w=seq2Hilbert(q,sdim=[[4,4],j])
    print(w)
    w=seq2Morton(q,sdim=[[4,4],j])
    print(w)
    w=seq2Snake(q,sdim=[[4,4],j])
    print(w)
    ...
    q=Seq('AATAGACAGACAGATA')
    w=seq2Simple(q,sdim=[[20],1,['A','T','C','G']])
    print(w)
