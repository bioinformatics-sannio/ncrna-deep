...
fs = dir("../RFAM/",pattern = "^R.*fa")
length(fs)
allseq = DNAStringSet()
for(fi in fs) {
  seqi = readDNAStringSet(paste0('../RFAM/',fi))
  names(seqi) = rep(substr(fi,1,7),length(seqi))
  allseq = c(allseq,seqi)
}


w = width(allseq)
q = names(allseq)
sdata = data.frame(length=w,family=q,stringsAsFactors = F)

clsize=aggregate(length~family,sdata,FUN=length)
tinycls = clsize$family[clsize$length<=30]
length(tinycls)
allseq = allseq[!names(allseq) %in% tinycls]

sdata = sdata[!sdata$family %in% tinycls,]


library(caret)
require(C50)
sdata$family = as.factor(sdata$family)
...
initialtot = length(allseq)
cat("total seqs:",initialtot)
cat("total classes:",length(unique(c(names(allseq)))))

allseq = allseq[!names(allseq) %in% fakeclass]

cat("removed %:",1-length(allseq)/initialtot)
cat("total seqs:",length(allseq))
cat("total classes:",length(unique(c(names(allseq)))))


cat("Remove sequences with letters different from canonical A T C and G\n")
ca=vcountPattern('A',allseq)
ct=vcountPattern('T',allseq)
cc=vcountPattern('C',allseq)
cg=vcountPattern('G',allseq)
w=ca+ct+cc+cg
...
allseq=allseq[width(allseq)<=maxlen & width(allseq)>=minlen]
...
clsize=aggregate(length~family,sdata,FUN=length)
smallcls = clsize$family[clsize$length<=400]

initialtot = length(allseq)
cat("total seqs:",initialtot)
cat("total classes:",length(unique(c(names(allseq)))))

allseq = allseq[!names(allseq) %in% smallcls]

cat("removed %:",1-length(allseq)/initialtot)
cat("total seqs:",length(allseq))
cat("total classes:",length(unique(c(names(allseq)))))

plot(density(sdata$length))
...
require(stringdist)
part = c()
cls = unique(c(names(allseq)))
for(ci in cls) {
  print(ci)
  idx = which(names(allseq) == ci)
  if (length(idx)> 10000) {
    idx = sample(idx,10000)
  }
  dm = stringdistmatrix(allseq[idx],method = "lv")
  p = cutree(hclust(dm,method = "complete"),k=2)
  if (sum(p==2) > sum(p==1)) {
    part = c(part,list(test=idx[p==1],train=idx[p==2]))
  } else {
    part = c(part,list(test=idx[p==2],train=idx[p==1]))
  }
}
